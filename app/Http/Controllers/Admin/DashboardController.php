...
class DashboardController extends Controller
{
    public function index(){
        ...
        $recent_orders=Order::latest()->select('id','customer_id','invoice_no','total','status')->take(10)->with(['customer:id,name','order_items.product'])->get();
        $balance=Balance::with('today_credit_balance','total_credit_balance','today_debit_balance','total_debit_balance')->get();
       
        return view('admin.index',compact(['analysis','balance','top_selling_products_this_week']));
